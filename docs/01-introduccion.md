...
Antes de definir las pruebas clasificamos Atlas según los tres ejes vistos en la asignatura:
...
Por esta clasificación, el plan no se limita a comprobar que las funciones operen. También verifica que se respete la confidencialidad entre empresas, la integridad de los contratos y la trazabilidad que exige la normativa de protección de datos.

## 1.3 Propósito del plan de pruebas

Este plan define cómo vamos a verificar que Atlas cumple sus requerimientos funcionales, no funcionales y reglas de negocio, y cómo cada verificación se relaciona con los estándares de calidad y el marco legal. Buscamos asegurar que:
...
**Fuera del alcance de esta iteración:** pruebas sobre la infraestructura física del proveedor de hosting, el servicio de correo externo más allá de su integración, y auditorías legales formales, que corresponden a un asesor jurídico. Estos puntos quedan registrados como riesgos y dependencias.

## 1.5 Estructura del informe

El informe se organiza así: criterios de calidad, seguridad y cumplimiento normativo (sección 2); tipos de prueba y su justificación (sección 3); estrategia de pruebas (sección 4); criterios de aceptación y herramientas (secciones 5 y 6); recursos y cronograma (secciones 7 y 8); diseño de casos de prueba (sección 9); matriz de trazabilidad entre requerimientos, pruebas y controles normativos (sección 10); y análisis de cobertura y coherencia del plan (sección 11).
